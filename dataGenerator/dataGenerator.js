...
var createHouse = () => {
  (async() => {
    houseData.write('id,title,is_entire_place,location,super_host_name,super_host_photo, rating,desc,space_desc,guest_desc,other_desc\n');
    for (var i = 1; i <= records; i++) {
        ...
        const House = `${id},${title},${is_entire_place},${location},${super_host_name},${super_host_photo},${rating},${desc},${space_desc},${guest_desc},${other_desc}\n`;
...
var createRooms = () => {
  (async() => {
    roomData.write('id,house_id,guests,size,bath\n');
    for (var i = 1; i <= records * 2; i++) {
      ...
      const Room = `${id},${house_id},${guests},${size},${bath}\n`;
